...
class EventItemPopover extends Component {
  ...
  render() {
    ...
    const start = localeMoment(startTime); const end = localeMoment(endTime);
    ...
    if (eventItemPopoverTemplateResolver) {
      ...
      let opsRow = <div/>;
      if (viewEventText && viewEventClick && (eventItem.clickable1 === undefined || eventItem.clickable1)) {
        let col = (
          <Col span={22}>
        ...
        if (viewEvent2Text && viewEvent2Click && (eventItem.clickable2 === undefined || eventItem.clickable2)) {
          col = (
            <Col span={22}>
              <span className="header2-text" style={{ color: '#108EE9', cursor: 'pointer' }} onClick={() => {
                viewEventClick(schedulerData, eventItem);
