...
import { ViewTypes, CellUnits, DATE_FORMAT, DATETIME_FORMAT } from '..'
...
export class SchedulerData {
	...
	public viewType: any;
	public cellUnit: any;
	public showAgenda: any;
	public isEventPerspective: any;
	public resizing: any;
  ...
  getSchedulerWidth() {
    ...
    return this.isSchedulerResponsive() ? Math.round(baseWidth * Number(this.config.schedulerWidth.slice(0, -1)) / 100) : this.config.schedulerWidth;
  }
  ...
  getResourceTableWidth() {
    ...
    let resourceTableWidth = this.isResourceViewResponsive() ? Math.round(schedulerWidth * Number(resourceTableConfigWidth.slice(0, -1)) / 100)
  ...
  getContentCellWidth() {
    ...
    const schedulerWidth = this.getSchedulerWidth();
    return this.isContentViewResponsive() ? Math.round(schedulerWidth * Number(contentCellConfigWidth.slice(0, -1)) / 100) : contentCellConfigWidth;
  }
  ...
  _compare(event1: any, event2: any) {
    const start1 = this.localeMoment(event1.start), start2 = this.localeMoment(event2.start);
    if (start1 !== start2) return start1 < start2 ? -1 : 1;

    const end1 = this.localeMoment(event1.end), end2 = this.localeMoment(event2.end);
