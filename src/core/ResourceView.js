...
class ResourceView extends Component {
    ...
    render() {
        ...
        const resourceList = displayRenderData.map((item) => {
            ...
            const a = slotClickedFunc ? <span className="slot-cell">{indents}<a className="slot-text" onClick={() => {
                slotClickedFunc(schedulerData, item);
            }}>{item.slotName}</a></span>
                : <span className="slot-cell">{indents}<span className="slot-text"><TechnicianCard name={item.slotName} /></span></span>;
            let slotItem = (
